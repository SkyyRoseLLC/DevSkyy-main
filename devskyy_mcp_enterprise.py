...
try:
    ...
    from mcp.types import CallToolResult, TextContent  # noqa: F401 - may be used dynamically
    from pydantic import BaseModel, ConfigDict, Field  # noqa: F401 - ConfigDict used for model config
except ImportError:
    sys.exit(1)
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[DevSkyyAppContext]:
    ...
    agents_prompt = agents_prompt_path.read_text() if agents_prompt_path.exists() else "# Agents directory not found"

    # Warmup API connection
    with suppress(Exception):
        await http_client.get("/health", timeout=5.0)
